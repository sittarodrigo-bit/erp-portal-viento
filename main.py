...
def locales_historial_producto(id_local: int, nombre_producto: str, desde: Optional[str] = None, hasta: Optional[str] = None):
    ...
    try:
        cur = conn.cursor(cursor_factory=RealDictCursor)
        q = """
            SELECT v.fecha::text, d.cantidad, d.cantidad * d.precio_unitario AS total,
...
@app.delete("/api/reparto/clientes/{id}")
def reparto_eliminar_cliente(id: int):
    """Eliminar un cliente de reparto (baja lógica)."""
    conn = obtener_conexion()
    try:
        cur = conn.cursor()
        cur.execute("DELETE FROM reparto_clientes WHERE id=%s", (id,))
        conn.commit()
        return {"status": "ok"}
    except Exception as e:
        conn.rollback()
...
def reparto_login(data: dict = Body(...)):
    ...
    try:
        ...
        try:
            ...
            cur.execute("INSERT INTO reparto_sesiones (id_empleado, nombre) VALUES (%s,%s)", (u['id_empleado'], nombre_completo.strip()))
            conn.commit()
        except Exception:
            conn.rollback()
...
@app.post("/api/reparto/logout")
def reparto_logout(data: dict = Body(...)):
    conn = obtener_conexion()
    try:
        cur = conn.cursor()
        id_emp = data.get('id_empleado')
        if id_emp:
            cur.execute("UPDATE reparto_sesiones SET activa=false, fin=NOW() WHERE id_empleado=%s AND activa=true", (id_emp,))
            conn.commit()
        return {"status": "ok"}
    finally:
        liberar_conexion(conn)
...
def catalogo_obtener(id_vendedor: int):
    """Obtener catálogo de un vendedor (SOLO productos marcados como visibles)."""
    conn = obtener_conexion()
    try:
        cur = conn.cursor(cursor_factory=RealDictCursor)
...
def notificaciones_obtener_sin_leer(id_empleado: Optional[int] = None):
    """Obtener notificaciones sin leer del empleado."""
    conn = obtener_conexion()
    try:
        cur = conn.cursor(cursor_factory=RealDictCursor)
        cond, params = "", []
...
def catalogo_pedido_cambiar_estado(id: int, estado: str = Body(...)):
...
@app.get("/api/catalogo/{id_vendedor}/productos-admin")
def catalogo_productos_admin(id_vendedor: int):
    """Obtener lista de productos para ADMIN (incluye visible/oculto)."""
    conn = obtener_conexion()
    try:
        cur = conn.cursor(cursor_factory=RealDictCursor)
        ...
        cur.execute("""
            SELECT 
                ...
                p.precio_minorista,
                p.imagen_url,
                COALESCE(cpv.visible, false) as visible,
                COALESCE(cpv.orden, 0) as orden,
                COALESCE(cpv.id, 0) as config_id
            FROM productos p
            LEFT JOIN catalogo_productos_visibles cpv 
                ON p.id = cpv.id_producto AND cpv.id_vendedor = %s
            WHERE COALESCE(p.activo, true) = true
            ORDER BY COALESCE(cpv.orden, 999), p.nombre
        """, (id_vendedor,))
...
@app.post("/api/catalogo/{id_vendedor}/productos/{id_producto}/visible")
def catalogo_toggle_visible(id_vendedor: int, id_producto: int, data: dict = Body(...)):
    """Cambiar si un producto es visible en el catálogo del vendedor."""
    conn = obtener_conexion()
    try:
        cur = conn.cursor()
        ...
        cur.execute("""
            INSERT INTO catalogo_productos_visibles (id_vendedor, id_producto, visible, orden)
            VALUES (%s, %s, %s, %s)
            ON CONFLICT (id_vendedor, id_producto) 
            DO UPDATE SET visible = %s, orden = %s
        """, (id_vendedor, id_producto, visible, orden, visible, orden))
