...
def historial_cajas(id_sucursal: Optional[int] = None):
    conn = obtener_conexion()
    try:
        cur = conn.cursor()
        filtro = "WHERE c.id_sucursal = %s" if id_sucursal else ""
        params = (id_sucursal,) if id_sucursal else ()
        cur.execute(f"""
            SELECT c.id, c.fecha_apertura::text, c.fecha_cierre::text,
                   c.monto_apertura, c.monto_cierre,
                   c.total_efectivo, c.total_tarjeta, c.estado, c.observaciones,
                   s.nombre as sucursal,
                   ea.nombre as apertura_nombre, ea.apellido as apertura_apellido,
                   ec.nombre as cierre_nombre, ec.apellido as cierre_apellido
            FROM cajas c
            JOIN sucursales s ON c.id_sucursal = s.id
            JOIN empleados ea ON c.id_empleado_apertura = ea.id
            LEFT JOIN empleados ec ON c.id_empleado_cierre = ec.id
            {filtro}
...
# INSUMOS — ver endpoints actualizados más abajo
    ...
    try:
        ...
        cur.execute("""
            SELECT p.id, p.sku, p.nombre, p.tipo, p.stock_actual as stock,
                   p.id_categoria, p.stock_alerta, p.imagen_url,
...
def crear_producto(prod: NuevoProducto):
    ...
    try:
        cur = conn.cursor()
        cur.execute("INSERT INTO productos (sku, nombre, tipo, stock_actual, id_categoria, stock_alerta, imagen_url) VALUES (%s,%s,%s,%s,%s,%s,%s) RETURNING id",
                    (prod.sku, prod.nombre, prod.tipo, prod.stock_inicial, prod.id_categoria, prod.stock_alerta, prod.imagen_url))
...
# Endpoint para catálogo completo con presentaciones (POS y B2B)
...
def crear_distribuidor(dist: NuevoDistribuidor):
    ...
    try:
        ...
        cur.execute("""
            ...
            (razon_social, cuit, limite_credito, dni, telefono, email, direccion, localidad, provincia, cp)
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
        """, (dist.razon_social, dist.cuit, dist.limite_credito, dist.dni, dist.telefono,
              dist.email, dist.direccion, dist.localidad, dist.provincia, dist.cp))
        conn.commit()
        return {"status": "ok"}
    except Exception as e:
        conn.rollback()
...
@app.put("/api/distribuidores/{id_dist}")
def actualizar_distribuidor(id_dist: int, dist: NuevoDistribuidor):
    conn = obtener_conexion()
    try:
        cur = conn.cursor()
        cur.execute("""
            UPDATE distribuidores SET
            razon_social=%s, cuit=%s, limite_credito=%s, dni=%s, telefono=%s,
            email=%s, direccion=%s, localidad=%s, provincia=%s, cp=%s
            WHERE id=%s
        """, (dist.razon_social, dist.cuit, dist.limite_credito, dist.dni, dist.telefono,
              dist.email, dist.direccion, dist.localidad, dist.provincia, dist.cp, id_dist))
        conn.commit()
        return {"status": "ok"}
    except Exception as e:
        conn.rollback()
...
def actualizar_producto(id_producto: int, prod: ActualizarProducto):
    ...
    try:
        cur = conn.cursor()
        cur.execute("""
            UPDATE productos SET sku=%s, nombre=%s, tipo=%s, id_categoria=%s, stock_alerta=%s, imagen_url=%s
            WHERE id=%s
...
@app.get("/api/reportes/horas")
def reporte_horas(fecha_desde: str, fecha_hasta: str, id_empleado: Optional[int] = None):
    conn = obtener_conexion()
    try:
        cur = conn.cursor()
        filtro_emp = "AND r.id_empleado = %s" if id_empleado else ""
        params = [fecha_desde, fecha_hasta]
        if id_empleado: params.append(id_empleado)

        cur.execute(f"""
            SELECT
                e.id as id_empleado,
                e.nombre, e.apellido, e.rol,
                DATE(r.fecha_hora) as dia,
                MIN(CASE WHEN r.tipo = 'entrada' THEN r.fecha_hora END) as entrada,
                MAX(CASE WHEN r.tipo = 'salida'  THEN r.fecha_hora END) as salida,
                EXTRACT(EPOCH FROM (
                    MAX(CASE WHEN r.tipo = 'salida'  THEN r.fecha_hora END) -
                    MIN(CASE WHEN r.tipo = 'entrada' THEN r.fecha_hora END)
                )) / 3600.0 as horas_dia
            FROM registros_horarios r
            JOIN empleados e ON r.id_empleado = e.id
            WHERE DATE(r.fecha_hora) BETWEEN %s AND %s
            {filtro_emp}
            GROUP BY e.id, e.nombre, e.apellido, e.rol, DATE(r.fecha_hora)
            ORDER BY e.apellido, e.nombre, dia DESC
        """, params)
        return fetchall_dict(cur)
    finally:
        liberar_conexion(conn)

# ── EMPLEADOS — EDITAR Y DAR DE BAJA ────────────────────────
...
def editar_empleado(id_emp: int, data: EditarEmpleado):
    conn = obtener_conexion()
    try:
        cur = conn.cursor()
        cur.execute("""
            UPDATE empleados SET nombre=%s, apellido=%s, dni=%s, rol=%s, email=%s, telefono=%s
            WHERE id=%s
        """, (data.nombre, data.apellido, data.dni, data.rol, data.email, data.telefono, id_emp))
        conn.commit()
        return {"status": "ok"}
    except Exception as e:
        conn.rollback()
...
@app.delete("/api/empleados/{id_emp}")
def dar_baja_empleado(id_emp: int):
    conn = obtener_conexion()
    try:
        cur = conn.cursor()
        cur.execute("UPDATE empleados SET activo=false WHERE id=%s", (id_emp,))
        conn.commit()
        return {"status": "ok"}
    finally:
        liberar_conexion(conn)
...
# ═══════════════════════════════════════════════════════════════
#  PORTAL DISTRIBUIDORES — LOGIN Y REGISTRO
# ═══════════════════════════════════════════════════════════════

class RegistroDistribuidor(BaseModel):
    razon_social: str
    cuit: str
...
class LoginDistribuidor(BaseModel):
    username: str
    password: str

@app.post("/api/distribuidores/registro")
def registro_distribuidor(data: RegistroDistribuidor):
    conn = obtener_conexion()
    try:
        cur = conn.cursor()
        # Verificar username único
        cur.execute("SELECT id FROM distribuidores WHERE username=%s", (data.username,))
        if cur.fetchone():
            raise HTTPException(status_code=400, detail="Ese nombre de usuario ya está en uso.")
        hash_pw = bcrypt.hashpw(data.password.encode(), bcrypt.gensalt()).decode()
        cur.execute("""
            INSERT INTO distribuidores
            (razon_social, cuit, limite_credito, dni, telefono, email,
             direccion, localidad, provincia, cp, username, password_hash, aprobado, activo)
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,false,true)
            RETURNING id, razon_social, cuit, dni, telefono, email,
                      direccion, localidad, provincia, cp, limite_credito, aprobado
        """, (data.razon_social, data.cuit, data.limite_credito, data.dni,
              data.telefono, data.email, data.direccion, data.localidad,
              data.provincia, data.cp, data.username, hash_pw))
        row = fetchone_dict(cur)
        conn.commit()
        return row
    except HTTPException: raise
...
def login_distribuidor(data: LoginDistribuidor):
    conn = obtener_conexion()
    try:
        cur = conn.cursor()
        cur.execute("""
            SELECT id, razon_social, cuit, dni, telefono, email,
                   direccion, localidad, provincia, cp, limite_credito,
                   aprobado, password_hash, activo
            FROM distribuidores WHERE username=%s
        """, (data.username,))
        dist = fetchone_dict(cur)
        if not dist or not dist['activo']:
            raise HTTPException(status_code=401, detail="Usuario o contraseña incorrectos.")
        if not dist['password_hash']:
            raise HTTPException(status_code=401, detail="Esta cuenta no tiene acceso al portal.")
        if not bcrypt.checkpw(data.password.encode(), dist['password_hash'].encode()):
            raise HTTPException(status_code=401, detail="Usuario o contraseña incorrectos.")
        dist.pop('password_hash', None)
        return dist
    finally:
        liberar_conexion(conn)
...
def estado_cuenta_distribuidor(id_dist: int):
    ...
    try:
        ...
        # Pedidos despachados
        cur.execute("""
            SELECT id, fecha::text, total, estado
            FROM pedidos_b2b
        ...
        # Cobros registrados
        cur.execute("""
            SELECT c.id, c.fecha::text, c.monto, c.metodo, c.referencia, c.id_pedido,
                   e.nombre as empleado_nombre, e.apellido as empleado_apellido
